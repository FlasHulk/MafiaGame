...
(function ($) {
  ...
  $('.distribute-roles').on('click', function () {
    if (players.length > 6) {
      var roles = generateRole(players.length);
      players.forEach(function (player) {
        player.role = String(roles.splice(Math.floor(Math.random() * roles.length), 1));
      });
      document.querySelectorAll('.players-role__player .player-role-select').forEach(function (item, key) {
  ...
  body.on('blur', '.players-role__player input[name="name"]', function () {
    var playerId = $(this).closest('.players-role__player').data('id');
  ...
  body.on('click', '.delete-player', function () {
    var playerWrap = $(this).closest('.players-role__player');
    playerWrap.remove();
    deletePlayer(playerWrap.data('id'));
    console.log(players);
  });
  $(document).on('mouseup', function (event) {
    var role_list = $(".player-role__list");

    if (!role_list.is(event.target) && role_list.has(event.target).length === 0) {
  ...
  function generateRole(amountPlayers) {
    ...
    var peacefulPlayer = amountPlayers - roles.length;
    pushRole(roles, "Мирный", peacefulPlayer); // function recursion push role
